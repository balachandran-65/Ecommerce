<div>
  @if ($products)    <div class="grid grid-cols-3 px-10 mt-5 product_container gap-9" >
        ...
        $color_count=0;
        $size=[];
        $color=[];        $size=$detail->product_size->select('size','id')->distinct()->get();
        $color=$detail->product_color->select('color','id')->distinct()->get();        @endphp
        @endforeach        <div class="py-5 product_item " x-data="{
        current_size:1,
        current_color:1,
        price:'',
        not_login:false,
        update(){            fetch(`http://127.0.0.1:8000/price/{{$product->id}}/${this.current_size}/${this.current_color}`)
            .then(data => data.json())
            .then(data=>{
            ...
            <div class="z-30 -mt-10 isolate">
                <div class="price_container p-2 flex items-center justify-between bg-[#1E2832
                    ...
                    @auth
                     ...
                     @else                     @click="not_login= !not_login"
                    @endauth
                    >                        <img src="../bag.svg" alt="">
                    ...
                    <div class="flex items-center justify-between gap-6 py-1 ">
                        <div class="size_container ">
                            ...
                            <div class="flex gap-2 size_list">
                                ...
                                <div   class="px-2 py-1 mt-2 border border-black rounded-full cursor-pointer w-fit"                                :style="current_size=={{$size_item->id}}? 'background-color:black;color:white' : 'background-color:white;color:black'"                                @click="current_size={{$size_item->id}}">{{$size_item->size}}</div>
                                @php
                                $size_count++;
                                if ($size_count==4) {
                                break;
                        ...
                        <div class="color_container ">
                            ...
                            <div class="flex items-center gap-2 color_list">
                                ...
                                <div  class="p-1 mt-2 rounded-full cursor-pointer "
                                    style="background-color: {{$color_item->color}}"                                    @click="current_color='{{$color_item->id}}'">
        ...
        @endforeach        @if (session()->has('cart_exist'))
        <div>
            <p x-data="{
                show:true,
                init(){
